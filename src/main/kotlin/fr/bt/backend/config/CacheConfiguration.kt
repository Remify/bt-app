...
class CacheConfiguration(jHipsterProperties: JHipsterProperties) {
    ...
    fun cacheManagerCustomizer(): JCacheManagerCustomizer {
        return JCacheManagerCustomizer { cm ->
            createCache(cm, fr.bt.backend.repository.UserRepository.USERS_BY_LOGIN_CACHE)
            createCache(cm, fr.bt.backend.repository.UserRepository.USERS_BY_EMAIL_CACHE)
            createCache(cm, fr.bt.backend.domain.User::class.java.name)
            createCache(cm, fr.bt.backend.domain.Authority::class.java.name)
            createCache(cm, fr.bt.backend.domain.User::class.java.name + ".authorities")
